...
public class DefaultTagTypeDetector<R> implements TagTypeDetector<R> {
	...
	public TagType parseHistoricalBytes(R reader, byte[] historicalBytes) {
		if (historicalBytes[0] == CATEGORY_COMPACT_TLV) {
			// The coding of the COMPACT-TLV data objects is deduced from the basic encoding rules af ASN.1 (see ISO/IEC 8825 and annex D) for BER-TLV data
			// objects with tag='4X' and length='0Y'. The coding of such data objects is replaced by 'XY' followed by 'Y' bytes of data. In this clause, 'X' is
			// referred to as the tag number and 'Y' as the length.
			// Besides the data objects defined in this clause, the historical bytes may contain data objects defined in part 4 of ISO/IEC 7816. In this case
			// the coding of the tags and length fields defined in part 5 shall be modified as above.
			// When COMPACT-TLV data objects defined in this clause appear in the ATR file, they shall be encoded according to the basic encoding rules of ASN.1
			// (i.e tag='4X', length='0Y').
